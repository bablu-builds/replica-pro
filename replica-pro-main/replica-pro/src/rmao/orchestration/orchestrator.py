...
class Orchestrator:
    ...
    async def run(
        ...
        active_cancel_event = cancel_event or self.cancel_events.get(run_id) or asyncio.Event()
        self.cancel_events[run_id] = active_cancel_event
        summary = self.runs.get(run_id) or OrchestratorRunSummary(
            run_id=run_id,
            state=RunState.planning,
        )
        summary.state = RunState.planning
    ...
    def start_run(self, request: str, task_count: int | None = None) -> str:
        """Schedule a run and return its id without waiting for completion."""
        run_id = f"run-{uuid.uuid4().hex[:12]}"
        self.runs[run_id] = OrchestratorRunSummary(
            run_id=run_id,
...
def build_orchestrator(config: RMAOConfig, task_count: int | None = None) -> Orchestrator:
    ...
    if config.execution.provider == "mock":
        ...
        from ..providers.replit import ReplitProvider
