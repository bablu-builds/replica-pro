...
- `replica-pro/src/rmao/execution/` — mock execution and Replit execution adapter
- `replica-pro/src/rmao/providers/` — async Replit GraphQL provider with session rotation
- `replica-pro/src/rmao/ui/dashboard.py` — authenticated Streamlit monitoring dashboard
...
- Replit automation is isolated behind a provider because its GraphQL endpoint is
  an internal API and may change independently.
- GitHub publishing is provider-neutral; real mode uses the GitHub REST API and mock mode stays in memory.
- The RMAO service uses FastAPI with slowapi rate limiting and JSON error responses.
